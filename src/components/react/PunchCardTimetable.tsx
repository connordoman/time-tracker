import type { PunchCardData, PunchCardInterval } from "@lib/time";
import TimeTracker, {
  timeInTimezone,
  hhMMSS,
  duration,
          ...
          return (
            <TableRow key={`${index}_${period.startTimeSeconds}`}>
              ...
              <TableCell className="text-center w-min text-xs">
                <Tooltip content="Delete" color="danger" delay={500}>
                  <Chip
                    as={Button}
                    color="danger"
                    variant="flat"
...
export default forwardRef((props: PunchCardTimetableProps, ref) =>
  PunchCardTimetable(props),
);
